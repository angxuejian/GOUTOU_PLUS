...
import {FILE_URL, CLOUD_FILE_ID} from '../../../utils/config'
Page({
  ...
  data: {
    ...
    lessPrice: null, // doc上的值
    truePrice: null, // 真实抵扣的钱,
    formData: {
      goods_type_id:'', // 商品分类
      less_price: 0, // 积分抵扣的值
      count:0, // 销量
  ...
  // 选择驾校分类
  getPickerCarValue: function(event) {
    const item = this.data.goodsCarTypeArr[event.detail.value]
    this.data.goodsCarType = item.name
    this.data.formData.is_car = item.value
    this.setData({
      goodsCarType: this.data.goodsCarType
    })
  },

  // 选择商品分类
  getPickerValue: function(event) {
    const item = this.data.goodsTypeArr[event.detail.value]
    this.data.goodsType = item.name
    this.data.formData.goods_type_id = item._id
    this.setData({
  ...
  getGoodsInfo: function () {
    ...
    cloudFn.$callFn({
      ...
      this.data.goodsTypeArr.forEach(item => {
        if(item._id === goods.goods_type_id) {
          this.data.goodsType = item.name
          this.data.formData.goods_type_id = item._id
        }
      })
      this.data.cover = goods.cover
      
      this.data.title = goods.title
      this.data.arrPrice = goods.def_price.split('-')
      this.data.arrSpec = goods.spec_array

      this.data.arrDetail = goods.detail_array
      this.data.arrDetail.push('')

      this.data.arrSwiper = goods.swiper_array
      this.data.arrSwiper.push('')

      this.data.goodsCarType = goods.is_car ? '是' : '否'
      this.data.formData.is_car = goods.is_car

      wx.hideLoading()
      this.setData({
        goodsCarType: this.data.goodsCarType,
        truePrice: goods.less_price ? `抵扣人民币${goods.less_price / 10 }元`: null,
        lessPrice: goods.less_price,
        goodsType: this.data.goodsType,
    ...
    cloudFn.$callFn({
      ...
      if(this.data.arrDelete.length) {
        ...
        if (this.data.errData.length) {
          this.onShowModal(`更新成功， 但有${ this.data.errData.length }项图片错误，未上传，请自行检查`)
        } else {
  ...
  beforeSave: function () {
    ...
    // 判断是否为空
    if(!this.data.formData.is_car) {
      this.onShowToast('请选择是否是驾校商品')
      return
    }
    if(!this.data.formData.goods_type_id) {
      this.onShowToast('请选择商品分类')
      return
    }
    if (!this.data.cover) {
    ...
    return new Promise((resolve, reject) => {
      if (item.cloud) {
        ...
        if (item.key === 'spec_array') {
          self.data.formData[item.key][self.data.innerIndex].cover = item.list[self.data.innerIndex]
        } else {
        ...
        wx.getFileSystemManager().readFile({
          ...
          success: res => {
            cloudFn.$callFn({
              ...
              if (item.key === 'spec_array') {
                self.data.formData[item.key][self.data.innerIndex].cover = png
              } else {
  ...
  clearData: function() {
    ...
    this.data.formData.spec_array.forEach(item => {
      item.price = Number(item.price) === NaN ? 0 : Number(item.price)
      item.sum = Number(item.sum)
    })

    // 积分
    this.data.formData.less_price = this.data.lessPrice
  ...
  removeImage: function (event) {
    ...
    */
    ...
    if (index === null) {
      this.data.arrDelete.push(this.data[key])
      this.data[key] = ''
      this.setData({
        [key]: this.data[key]
      })
      ...
      if (subkey === null) {
        ...
        this.data.arrDelete.push(this.data[key][index][subkey])
  ...
  // 获取积分抵扣的值
  getLessPrice: function(event){
    const { value } = event.detail
    this.data.lessPrice = Number(value)
    this.setData({
      truePrice: value ? `抵扣人民币${value / 10 }元`: null
    })
