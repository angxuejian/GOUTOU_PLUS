...
Page({
  ...
  // 选择驾校分类
  getPickerAdValue: function(event) {
    const item = this.data.goodsAdTypeArr[event.detail.value]
    this.data.goodsAdType = item.name
    this.data.formData.is_ad = item.value
    this.setData({
      goodsAdType: this.data.goodsAdType
    })
  },
  ...
  getGoodsInfo: function () {
    ...
    cloudFn.$callFn({
      ...
      this.data.goodsAdType = goods.is_ad ? '是' : '否'
      this.data.formData.is_ad = goods.is_ad
  ...
  beforeSave: function () {
    ...
    // 判断是否为空
    if(!this.data.formData.is_ad) {
      this.onShowToast('请选择是否为广告商品')
      return
    }
